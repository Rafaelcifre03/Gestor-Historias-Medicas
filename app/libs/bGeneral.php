...
function cNombre($usr, $campo = NULL, &$errores = NULL)
{
	if (preg_match("/^[a-zA-Z ]{3,32}$/", $usr)) {
		return true;
	} else {
		if (!is_null($campo) && !is_null($errores)) {
			$errores[$campo] = "Usuario no válido. Solo puede contener letras mayúsculas y minúsculas. Entre 3 y 32 carácteres.";
		}
		return false;
	}
}
...
function cContrasenas($pw1, $pw2, $campo = NULL, &$errores = NULL)
{
	...
	if (preg_match("/^[a-zA-Z0-9_]{3,32}$/", $pw1) && preg_match("/^[a-zA-Z0-9_]{3,32}$/", $pw2)) {
		return true;
	} else {
...
function cContrasena($pw1, $campo = NULL, &$errores)
{
	if (preg_match("/^[a-zA-Z0-9_]{3,32}$/", $pw1)) {
		return true;
	} else {
...
function cFecha($fecha, $campo="fecha", &$errores) {
	$fecha = explode("-", $fecha);

	if (count($fecha) < 3 || count($fecha) > 3) {
		$errores["campo"]="fecha no valida1";
	...
	if (checkdate($fecha[1], $fecha[2], $fecha[0]))
		...
		$errores[$campo] = "fecha no valida2";
		return false;
	}
}

function cSexo ($sexo, $campo="sexo", &$errores) {
	if ($sexo!="" && ($sexo == "M" || $sexo="F") )
		return true;
	else
	{
		$errores[$campo] = "sexo no valido";
		return false;
	}
}
